...
import { X, User, Building, Mail, Phone, Link, Briefcase, Loader2 } from "lucide-react";
import { addContactService } from "../../services/apiServices";
...
const AddContactModal = ({ isOpen, onClose, onSuccess }: AddContactModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await addContactService(callApi, payload);
      ...
      if (response?.success) {
        ...
        setError(response?.errMessage || "Failed to add contact");
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-lg bg-[#0D111A] border border-[#1E293B] rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 transform scale-100 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-3 pt-6">
            ...
            >
              {loading ? <Loader2 size={16} className="animate-spin" /> : "Save Contact"}
            </button>
          </div>
        </form>
      </div>
